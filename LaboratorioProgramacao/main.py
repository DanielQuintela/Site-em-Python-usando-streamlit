...
def banco_protocolo():
    cursor.execute('CREATE TABLE IF NOT EXISTS protocolos(justificativa TEXT,resumo_portugues TEXT, resumo_ingles TEXT,data_inicio TEXT,data_fim TEXT, especie TEXT, quantidade_animais TEXT, bioterio TEXT)')

def addbanco_protocolo(input_justificativa,input_resumopt,input_resumoig,input_datainicio,input_dataterm ,input_especie ,input_qntanimal,_bioterio ):
    cursor.execute('INSERT INTO protocolos(justificativa,resumo_portugues,resumo_ingles,data_inicio,data_fim,especie,quantidade_animais,bioterio) VALUES (?,?,?,?,?,?,?,?)', (input_justificativa,input_resumopt,input_resumoig,input_datainicio,input_dataterm ,input_especie ,input_qntanimal,_bioterio ))
    con.commit()

#cadastro bioterio

def banco_bioterio():
    cursor.execute('CREATE TABLE IF NOT EXISTS bioterio(nome TEXT, especies TEXT)')

def addbanco_bioterio(cadastro_biot):
    cursor.execute(f'INSERT INTO bioterio(nome) VALUES (?)', (cadastro_biot,))
    con.commit()
...
if paginaSelecionada == 'Tela de inicio':
    ...
    if funcionarios == 'Login':
        ...
        if st.sidebar.checkbox('Login'):
            ...
            if result:
                ...
                area_pesq =st.selectbox('Selecione o que deseja',['Emitir Protocolo','Receber protocolo'])
        ...
        if result:
            if login_secretaria:
                ...
                if y == 'Cadastrar Bioterios':
                    cadastro_biot = st.text_input(label = 'Insira o nome do Bioterio')
    ...
    if st.sidebar.checkbox('Login'):
        ...
        if result:
            ...
            secretaria = st.selectbox('Escolha a função', ['Inicio', 'Cadastro de Secretária','Analise de Relatorios'])
